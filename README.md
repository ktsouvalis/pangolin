...
- **User Emails**: comma-separated university emails. **Each email becomes its own site resource** — a row with 3 emails creates 3 resources, each granting access to only that one resolved user (`roleIds` is always empty — no role-based access). An email that doesn't match any current org user still gets its resource created (the name/niceId never depended on the lookup succeeding) but with no user attached and **created disabled** (`enabled: false`, see below) — reported as `OK_NO_USER`/`DRY-RUN_NO_USER` rather than skipped. Run `normalize_private_resources.py` later, once the account exists, to backfill access and flip it on. A row is only skipped locally (no API call, reported `FAIL`) for invalid `Ports` or an unparseable `Destination`, or if it has no emails at all.

Resource names are computed, not typed in: `<city>-<username>-<vlan>-<tail>`, where `city` is the (lowercased) `Name` column, `username` is the part of the email before `@` with any `.` stripped (e.g. `m.katsis` → `mkatsis`), and `vlan`/`tail` come from the Destination's `10.x.y.z` octets/prefix — host: `vlan` = `x`+zero-padded `y`, `tail` = unpadded `z`; CIDR: `vlan` built from whichever octets the mask fixes, `tail` `all` for a `/24` or the literal prefix length otherwise (same rule `normalize_private_resources.py` uses, see below). Example: `ktsouvalis@uop.gr` at `10.23.2.50`, city "Patra" → `patra-ktsouvalis-2302-50`.

`niceId` is set the same way, deterministically, at create time: `<username>-<vlan>-<tail>-<ports>` (ports from the row's own `Ports` column, sorted, each prefixed `p`) — regardless of whether the email resolved to a user, since the username segment only ever needed the raw email string. (This was briefly left to Pangolin's own random generation instead, 2026-07-31–2026-08-04, specifically so a still-random `niceId` could signal "nobody's assigned yet" — as of 2026-08-04, Pangolin's `enabled` switch is that explicit signal instead, so the trick is no longer needed.) `enabled` is set to whether the email actually resolved to a current org user — `true` if matched, `false` for an `OK_NO_USER`/`DRY-RUN_NO_USER` row.

Every run writes a `<input>_results_<timestamp>.xlsx` report next to the input file, one row per attempted resource (or per skipped email): `City`, computed `Name`, `Destination`, `Alias`, **TCP Ports** — the row's `Ports` value as parsed — `Email`, `Notes`, `Sites` it spans, `Status` (`OK` / `OK_NO_USER` / `FAIL` / `DRY-RUN` / `DRY-RUN_NO_USER`), `Nice ID` and `Enabled` (both computed up front, so both are populated on a dry run too, not just a live create), a `Timestamp` of the attempt, and any `Error` (including the no-user-match note or a row with no emails at all).
...
- 2+ users assigned → resolved only if exactly one of them has a sanitized email matching a segment of the existing name (i.e. the name already seems to identify a primary owner among several grantees); otherwise skipped as ambiguous. As of 2026-07-31, the *other* assigned users are no longer just dropped when this resource gets pruned down to the resolved owner — each one with a resolvable email gets **split off into their own new resource** first (same destination/ports, spanning every org site, named and `niceId`'d for just them, the same one-resource-per-user shape `create_private_resources.py` itself produces, and created enabled since the target is already resolved), so normalizing a legacy multi-user resource never silently revokes anyone's access. A user with no resolvable email (rare, on some legacy resources) has nothing to split to and is still just dropped. If creating any split resource fails, the original is left untouched rather than pruning access with nowhere for it to have gone.
- 0 users assigned → this is the expected state for a resource `create_private_resources.py` created for an email that didn't match any org user yet (its `OK_NO_USER` status, created disabled). Resolved and granted (`action=grant_access`, plus enabling the resource if it was disabled — no confirmation needed) when either: the name or `niceId` is an *exact* reconstruction of the naming convention (peeling the known city/vlan/tail, or vlan/tail/ports, off the name/niceId leaves exactly one org user's sanitized local part — the literal reverse of `create_private_resources.py`'s own naming formulas, not a guess); or, as of 2026-07-31, as a fallback when neither exact reconstruction applies, exactly one org user's sanitized local part appears as *any* standalone segment of the name (catches legacy/hand-created names that don't fit the computed shape at all). Anything looser still (zero or multiple org users match) is skipped instead, with a best-effort suggested email for a human to confirm and grant by hand — and, as of 2026-08-04, the resource is disabled (if not already) whenever it stays in this unresolved 0-user state, since its default Admin-role access would otherwise still be reachable by anyone holding that role while unowned.

It never grants or splits access on a genuinely ambiguous match — only a unique match (exact reconstruction, or a single unambiguous name segment) is auto-applied; anything looser is always reported and left alone. Enabled state follows access, not naming: a 2+-user resource with an ambiguous (unresolved) primary is left exactly as enabled/disabled as it already was, since it already has real users on it.

**`enabled` (Pangolin 1.21.1+, wired up 2026-08-04):** this script keeps it in sync with whether a target owner is actually resolved — the explicit signal that replaced the old "leave niceId random to mean unassigned" trick described above. Disabled for an unresolved 0-user resource, enabled for anything with a resolved target (0-user grant, single assignment, or 2+-user primary). Merged into the same update call as any rename/niceId fix when more than one applies.

**Confirmed live Pangolin API behavior worth knowing about:** an update (`POST /site-resource/{id}`) that omits `tcpPortRangeString`/`udpPortRangeString`/`disableIcmp` doesn't leave them alone — it resets `udpPortRangeString` from blocked back to `*` (all) on the resource being edited. This script never intends to touch ports/ICMP, so every update explicitly re-asserts the resource's own current values for these fields rather than omitting them. `enabled` is treated with the same caution (always explicitly sent on every update this script issues) even though its omit-on-update behavior hasn't been separately confirmed live yet.
...
Roles granted on a resource are read and reported but never modified — every resource in this org, including ones this tooling created, carries the org's baseline `Admin` role regardless of what's requested, which looks like a server-side default rather than per-resource state either script manages. Confirmed live: this role comes back from the API listed *twice* (same role, `isAdmin: true` — Pangolin's built-in super-admin, which is why it's invisible under a resource's own Roles tab in the dashboard), and that duplication is treated as expected — only a genuinely different role name is flagged as an anomaly in the report. `Clients` are confirmed always empty for every resource here and are no longer reported at all.
