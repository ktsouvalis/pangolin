...
- `create_private_resources.py` always sends `roleIds: []` and grants access only via a resolved single `userIds` entry per resource — that's deliberate (per-resource, per-user, email-based allowlists, no role-based access), not a gap to fill in. An unresolved email is skipped locally (reported as a FAIL row, no API call) rather than creating a resource with no user attached; other emails on the same row still get their resources created.
